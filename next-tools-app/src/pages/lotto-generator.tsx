...
import styles from '../components/LottoGenerator.module.css';

function getBallColor(n: number) {
  if (n <= 10) return styles.ballYellow;
  if (n <= 20) return styles.ballBlue;
  if (n <= 30) return styles.ballRed;
  if (n <= 40) return styles.ballGreen;
  return styles.ballPurple;
}
  ...
  return (
    <div className={styles.animContainer}>
      <div className={styles.dome} ref={domeRef}>
        <div className={styles.ballsAnim}>
          {ANIM_BALLS.map((ball, i) => (
            <div
              key={i}
              className={`${styles.ballAnim} ${ball.color}`}
              style={{
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.controls}>
        ...
        <button className={styles.btn} onClick={handleGenerate} disabled={isDrawing}>추첨하기</button>
        <button className={styles.btn} onClick={handleCopy} disabled={results.length === 0 || isDrawing}>복사하기</button>
        <button className={styles.btn} onClick={handleReset} disabled={results.length === 0 || isDrawing}>초기화</button>
      </div>
        ...
        {results.map((set, idx) => (
          <div className={styles.card} key={idx}>
            <div className={styles.numbers}>
              {set.numbers.map((n, i) => (
                <span className={`${styles.ball} ${getBallColor(n)}`} key={i}>{n}</span>
              ))}
              {set.bonus !== undefined && (
                <span className={`${styles.ball} ${getBallColor(set.bonus)}`} style={{ border: '2px dashed #007BFF', background: '#fff', color: '#007BFF', fontWeight: 700, marginLeft: 10 }}>
                  {set.bonus}
                </span>
              )}
            </div>
            <div className={styles.time}>생성: {set.time}</div>
          </div>
        ))}
      </div>
    </div>
